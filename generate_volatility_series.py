...
def generate_volatility_series(comments_file, outfile, max_len, embedding_dimension):
    ...
    preprocessed_data = []
    date_of_comments = []
    for (comment, day) in zip(comments, dates):
...
if __name__ == "__main__":
    ...
    for(comment_dir, result_dir) in zip(comments_list, results_list):
        generate_volatility_series(os.path.join('data/stock_comments', comment_dir),
                        os.path.join('data/votality_series', result_dir),
                        max_len = 30,
                        embedding_dimension = 20)